...
**Version**: 2.0 — See [v2 Changes](#v2-changes) at the bottom.
...
### 1.8 workplace_challenges
- **challenge_type** - Type of challenge (micromanagement, conflict, culture, growth)
- **description** - What the challenge is
- **impact** - How it affects them
- **coping** - How they're handling it

**Example messages:**
- "My manager micromanages everything I do"
- "The culture at my company is very toxic"
- "There's no room for growth in my current role"
- "I'm dealing with a difficult coworker situation"

### 1.9 job_search_status
...
- **expertise_domains** - Broader knowledge areas and domain expertise
...
- **academic_awards** - Academic honors and recognitions (Dean's List, Summa Cum Laude, scholarships)
...
- **mindset** - Growth vs. fixed mindset; beliefs about talent and how they approach challenges
...
- **decision_making** - How they make decisions (analytical/intuitive/collaborative)
- **communication_style** - How they communicate
- **ideal_environment** - What environment they thrive in
- **energizers** - What energizes them at work
...
- "I struggle to control my emotions under pressure"

### 4.6 stress_and_coping
...
- **major_life_events** - Significant life events and their impact (marriage, birth, move, health crisis, recovery)
...
### 6.1 greetings
- **greeting_type** - Hello, goodbye, how are you
...
### 6.2 thanks
- **gratitude_expression** - Expressing thanks or appreciation
...
### 6.3 small_talk
- **topic** - General non-career small talk
...
### 6.4 feedback
- **feedback_type** - Positive, negative, or clarification request about coach's response
- **content** - What they liked, disliked, or want clarified

**Example messages:**
- "That advice was really helpful, thank you!"
...
### 7.1 unrelated_topics
- **topic** - Topics unrelated to career coaching

**Example messages:**
- "Can you help me with my taxes?"
- "What's a good recipe for pasta?"

### 7.2 solicitation_or_misuse
- **request_type** - Content writing, homework, impersonation, or other misuse
- **description** - What they're asking the coach to do inappropriately
...
"I'm married with a young child",personal,personal_life,life_stage
```

### Classification Flow

1. **Level 1: Context Classification** (7 classes)
   - professional, learning, social, psychological, personal, chitchat, off_topic

2. **Level 2: Entity Classification** (context-specific)
   - For professional: current_position, professional_experience, professional_aspirations, publications, workplace_challenges, job_search_status, etc.
   - For personal: personal_life, health_and_wellbeing, living_situation, etc.
...
### v2.0 (2026-02-18) — Entity Merges, Promotions & Additions

**Professional (new entities):**
- **1.7 publications** — Moved from `learning.publications`. Publications are professional outputs, not learning activities.
- **1.8 workplace_challenges** — New entity. Captures micromanagement, toxic culture, lack of growth.
- **1.9 job_search_status** — Promoted from sub-entity of `professional_aspirations` (1.5.5) to standalone entity.

**Learning (merges & removals):**
- **2.1 current_skills** — Added `expertise_domains` field (absorbed from removed `knowledge_areas` entity).
- **2.3 education_history** — Added `academic_awards` field (absorbed from removed `academic_awards` entity).
- **Removed 2.7 knowledge_areas** — Merged into `current_skills.expertise_domains`.
- **Removed 2.10 publications** — Moved to `professional.publications`.
- **Removed 2.11 academic_awards** — Merged into `education_history.academic_awards`.

**Psychological (merges & removals):**
- **4.3 motivations** — Added `mindset` field (absorbed from removed `growth_mindset` entity).
- **4.4 working_style_preferences** — Added `decision_making`, `ideal_environment`, `energizers`, `stressors` fields (absorbed from removed `career_decision_making_style` and `work_environment_preferences` entities).
- **4.5 confidence_and_self_perception** — Added `4.5.5 emotional_awareness` sub-entity (absorbed from removed `emotional_intelligence` entity).
- **Removed 4.6 career_decision_making_style** — Merged into `working_style_preferences`.
- **Removed 4.7 work_environment_preferences** — Merged into `working_style_preferences`.
- **Removed 4.9 emotional_intelligence** — Merged into `confidence_and_self_perception` as `emotional_awareness`.
- **Removed 4.10 growth_mindset** — Merged into `motivations.mindset`.

**Personal (merges & removals):**
- **5.1 personal_life** — Added `major_life_events` field (absorbed from removed `major_life_events` entity).
- **Removed 5.10 major_life_events** — Merged into `personal_life.major_life_events`.
- **Removed 5.12 life_satisfaction** — Too abstract for reliable classification; satisfaction signals captured in other entities.

**Chitchat (new entity):**
- **6.4 feedback** — New entity. Captures user reactions to coach responses (positive, negative, clarification).

**Off-topic (new entity):**
- **7.2 solicitation_or_misuse** — New entity. Captures requests to write content, do homework, or impersonate AI.
