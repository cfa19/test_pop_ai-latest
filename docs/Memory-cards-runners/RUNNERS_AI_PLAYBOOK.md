...
| `preference` | "What do I like?" | `professional.work_preferences`, `personal.learning_preferences`, `personal.work_life_balance` |
...
professional: {
  ...
  // Market Awareness → memory type: competence
  market_awareness: {
    score: number;
  ...
  // Market Positioning → memory type: trait
  market_positioning: {
  ...
  // Validations → memory type: competence
  validations: {
  ...
  // Visibility → memory type: competence (visibility), aspiration (personal_branding)
  visibility: {
  ...
  // Competency Development → memory type: competence
  montee_competences: {
  ...
  // Career Clarity → memory type: aspiration
  career_clarity: {
  ...
  // Project Assessment → memory type: experience
  project: {
  ...
  // Work Preferences → memory type: preference
  work_preferences: {
    preferred_work_style?: string;        // 'remote' | 'hybrid' | 'onsite'
...
personal: {
  ...
  learning_preferences: {
    preferred_formats?: string[];         // e.g. ['video', 'hands-on', 'reading']
...
```typescript
market: {
  // Memory type: competence (research), trait (positioning)
  research: {
...
// autonomie (2 runners) → memory type: competence
autonomie: {
...
// mindset (1 runner) → memory type: emotion
mindset: {
...
| `journey_contributions.professional.unique_value_proposition` | `trait` | 0.85 | Tier 2 (deterministic) |
...
| `journey_contributions.professional.development_priorities` | `aspiration` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.professional.career_goals` | `aspiration` | 0.90 | Tier 2 (deterministic) |
| `journey_contributions.professional.priorities` | `aspiration` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.professional.mini_bilan` | `trait` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.professional.patterns` | `trait` | 0.80 | Tier 2 (deterministic) |
| `journey_contributions.professional.market_awareness` | `competence` | 0.80 | Tier 2 (deterministic) |
...
| `journey_contributions.professional.negotiation` | `competence` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.professional.objection_handling` | `competence` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.professional.visibility` | `competence` | 0.80 | Tier 2 (deterministic) |
| `journey_contributions.professional.personal_branding` | `aspiration` | 0.80 | Tier 2 (deterministic) |
| `journey_contributions.professional.montee_competences` | `competence` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.professional.job_assessments` | `experience` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.professional.career_clarity` | `aspiration` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.professional.project` | `experience` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.professional.activation_readiness` | `trait` | 0.80 | Tier 2 (deterministic) |
| `journey_contributions.professional.work_preferences` | `preference` | 0.85 | Tier 2 (deterministic) |
| **personal domain** | | | |
| `journey_contributions.personal.vision_6m` | `aspiration` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.personal.goals` | `aspiration` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.personal.keywords` | `trait` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.personal.readiness_signals` | `trait` | 0.80 | Tier 2 (deterministic) |
...
| `journey_contributions.personal.commitment` | `trait` | 0.80 | Tier 2 (deterministic) |
| `journey_contributions.personal.learning_preferences` | `preference` | 0.80 | Tier 2 (deterministic) |
| `journey_contributions.personal.work_life_balance` | `preference` | 0.80 | Tier 2 (deterministic) |
| **other domains** | | | |
| `journey_contributions.network.*` | `connection` | 0.80 | Tier 2 (deterministic) |
| `journey_contributions.market.research` | `competence` | 0.80 | Tier 2 (deterministic) |
| `journey_contributions.market.positioning` | `trait` | 0.80 | Tier 2 (deterministic) |
| `journey_contributions.decision.validation` | `trait` | 0.80 | Tier 2 (deterministic) |
| `journey_contributions.decision.market_validation` | `aspiration` | 0.80 | Tier 2 (deterministic) |
| `journey_contributions.skills.communication` | `competence` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.skills.interview` | `competence` | 0.85 | Tier 2 (deterministic) |
| `journey_contributions.autonomie` | `competence` | 0.80 | Tier 2 (deterministic) |
...
| **0.85-0.90** | Deterministic mapping from `journey_contributions` (high-specificity paths) | Tier 2 |
| **0.80** | Deterministic mapping from `journey_contributions` (broader paths) | Tier 2 |
...
### Content guardrails (Art. 9 GDPR — special category data)

Trait and emotion memory cards border on psychological profiling. To stay within career coaching (not clinical health data), runner developers must follow these content boundaries in `journey_contributions`:

```typescript
// ❌ WRONG: Clinical/diagnostic language (crosses into health data under Art. 9)
journey_contributions: {
  personal: {
    perceived_obstacles: ['Depression', 'Anxiety disorder', 'Possible ADHD'],
    emotional_state: 'Burnout syndrome — severe (8/10)',
  }
}

// ✅ CORRECT: Career-contextual observations only
journey_contributions: {
  personal: {
    perceived_obstacles: ['Manque de confiance en entretien', 'Peur du jugement'],
    emotional_state: 'Anxieux face aux entretiens, motivé par le changement',
  }
}
```
...
|---|---|---|
| No clinical diagnoses | "anxious about interviews" | "generalized anxiety disorder" |
| No diagnostic labels | "low confidence in public speaking" | "exhibits signs of clinical anxiety" |
| No severity scales | "motivated by creative work" | "depression score: 7/10" |
| Career-contextual only | "risk-averse in career decisions" | "avoidant personality pattern" |
| Observable behaviors | "strong communicator" | "possible ADHD traits" |

See `docs/DPIA-memory-card-extraction.md` section 5.7 for the full legal assessment.
...
**Consent flow:**
1. During the user's **first activity** (any runner), a consent modal is displayed listing all 7 insight types (skills, experiences, goals, preferences, traits, emotional state, connections). The user chooses "Enable AI personalization" or "Not now". Either choice writes to `user_consents` with `consent_type='ai_training'`. The activity starts immediately regardless of choice.
2. **One consent covers all runner types** (professional, psychological, motivational) — same purpose, all data types explicitly listed (GDPR Art. 7 + Art. 9 compliant).
3. For all subsequent activities, the consent modal never appears again — the AI backend **only reads** this table, never writes to it.
4. If no consent row exists yet (user hasn't started any activity), the check returns `false` (fail-closed).
5. User can change their choice at any time from profile settings (one-click toggle). If `consent_version` is bumped (new data categories added), the modal re-appears for all users.
...
### GAP 6 (MEDIUM): `dashboard_data` exists both inside `responses` AND as a separate column

`activity_completions` has a **separate** `dashboard_data` JSONB column (GIN-indexed for consultant queries), but `dashboard_data` also remains inside the `responses` JSONB (which contains the full runner output).

**AI_BACKEND_PLAN** reads `responses` from the database, which contains `dashboard_data` alongside all other runner output.
...
**AI_BACKEND_PLAN** originally proposed a separate `consent_audit_log` table.
...
**Fix**: Removed from AI_BACKEND_PLAN. Use existing tables.
...
**Design decision:** A consent modal is displayed before the user's **first activity** (any runner). One consent covers all runner types. The consent text explicitly lists all 7 insight types including psychological/emotional data (Art. 9 compliance). The activity works regardless of the user's choice (Art. 7 — freely given).
...
**GDPR compliance:**
- **Art. 7** (specific, informed, freely given): All 7 types listed, "Not now" lets user continue
- **Art. 9** (special category): "personality traits", "emotional state" explicitly named
- **Art. 22** (automated profiling): "automatically learn" describes the automated nature
- **One consent, all runners**: Same purpose (AI coaching), all data types listed — no per-runner consent needed
- **Consent versioning**: If new data categories added, bump `consent_version` and re-show modal
...
3. On "Enable": call `PUT /api/privacy/consents/ai_training` with `{ consent_given: true }`
4. On "Not now": call `PUT /api/privacy/consents/ai_training` with `{ consent_given: false }`
5. Must capture: consent text, `consent_version: '1.0'`, IP address, user agent (RGPD)
6. Once a consent row exists, the modal never appears again
7. User can change their choice from profile settings (one-click toggle)
...
|---|-----------|--------|--------|
| 1 | **Use existing `user_consents` table** — No separate consent tables needed. Query: `consent_type='ai_training' AND consent_given=true AND revoked_at IS NULL` | Avoids table conflicts | Applied to AI_BACKEND_PLAN |
...
| 9 | **BLOCKER: Add AI consent modal to first activity** — No code writes `ai_training` consent. `check_consent()` returns False for all users. Consent modal (listing all 7 insight types incl. psychological/emotional) shown once on first activity. One consent covers all runner types. Art. 7 + Art. 9 + Art. 22 compliant | Zero extraction for all users | Documented as hard blocker |
