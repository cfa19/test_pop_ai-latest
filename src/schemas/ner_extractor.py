...
PHRASE_DICT: dict[str, list[str]] = {
    ...
    "JOB_SEARCH_STATUS": [
        ...
        "on the job market",
        "in job search mode",
        "casually browsing opportunities",
        "passively looking",
...
def spans_to_fields(spans: list[dict], subcategory: str, message: str) -> dict:
    ...
    return {field: values[0] if len(values) == 1 else values for field, values in field_values.items()}
